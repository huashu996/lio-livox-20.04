...
void pubOdometry(const Eigen::Matrix4d& newPose, double& timefullCloud){
  ...
  pubLaserOdometryPath.publish(laserOdoPath);
	if (1){
		std::ofstream foutC(string("/home/cxl/code/LIO-SOTA/lio_livox/src/LIO-Livox-master/results/lio-livox.txt"), std::ios::app);
		foutC.setf(std::ios::fixed, std::ios::floatfield);
		foutC.precision(5);
		if (first_scan){
			base_timestamp = laserOdometry.header.stamp;
			save_timestamp = ros::Duration(0);
			first_scan = false;
		}
		else {
			save_timestamp = laserOdometry.header.stamp - base_timestamp;
			}
